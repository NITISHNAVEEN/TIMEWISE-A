...
import { Course, Faculty, Room, Timetable, TimetableConflict, Holiday, Cancellation, FacultyLeave, SemesterSettings, FeedbackItem, ExtraClass, TimetableEntry, Basket, ExamSchedule } from '@/lib/types';
...
export const TimetableProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [semesterSettings, setSemesterSettings] = useState<SemesterSettings>({
    ...
    seniorEndDate: add(new Date(), { months: 4 }),
  });
    ...
    const loadData = async () => {
        ...
        try {
            const collectionsToFetch = [
                ...
                { name: 'examSchedules', setter: setExamSchedules },
            ];
  ...
  useFirestoreSync('examSchedules', examSchedules);
  ...
  }, [courses, faculty, rooms, holidays, cancellations, facultyLeaves, semesterSettings, baskets, extraClasses, examSchedules, loading]);
  ...
  const revertExtraClass = (extraClassId: string) => {
    ...
    if (extraClassToRemove.linkedCancellationId) {
        ...
        if (linkedCancellation.reason.startsWith('FacultyLeave:') && extraClassToRemove.facultyId) {
            const reason = linkedCancellation.reason.replace('FacultyLeave: ', '');
            setFacultyLeaves(prev => prev.filter(fl => !(
                fl.facultyId === extraClassToRemove.facultyId &&
                fl.reason === reason &&
                fl.dateRange.from.getTime() <= linkedCancellation.date.getTime() &&
                (fl.dateRange.to || fl.dateRange.from).getTime() >= linkedCancellation.date.getTime()
            )));
        }
    }
  };
